...
IMForwardEventStruct *call_data;
{
    ...
    last_connectid = (int)call_data->connect_id;
    last_icid = (int)call_data->icid;

printf("last id:%x %x\n", last_icid, last_connectid);
...
printf("real commit: major_code: %x\n", ((IMCommitStruct*)call_data)->major_code);
printf("real commit: icid      : %x\n", ((IMCommitStruct*)call_data)->icid);
printf("real commit: connect id: %x\n", ((IMCommitStruct*)call_data)->connect_id);
printf("real commit: flag      : %x\n", ((IMCommitStruct*)call_data)->flag);
...
MySetFocusHandler(ims, call_data)
XIMS ims;
...
printf("proto handler:%d\n", call_data->major_code);
	...
	return MySetFocusHandler(ims, call_data);
...
void MyXEventHandler(im_window, ims, event)
Window im_window;
XIMS ims;
XEvent *event;
{
...
*/
    ...
    im_window = XCreateWindow(dpy, DefaultRootWindow(dpy),
			0, 700, 400, 800-700, 0,
	...
	MyXEventHandler(im_window, ims, &event);
    }
}
