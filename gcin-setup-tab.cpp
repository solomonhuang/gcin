...
COLORSEL colorsel[2] =
  { {&gcin_win_gcolor_fg, &gcin_win_color_fg, NULL, N_("前景顏色")},
    {&gcin_win_gcolor_bg, &gcin_win_color_bg, NULL, N_("背景顏色")}
  };
...
{
  char *restr = (char *)(res ? N_("結果失敗"):N_("結果成功"));
...
#if GTK_CHECK_VERSION(2,4,0)
   GtkWidget *file_selector = gtk_file_chooser_dialog_new(_("請輸入要匯出的檔案名稱"),
...
#if GTK_CHECK_VERSION(2,4,0)
   GtkWidget *file_selector = gtk_file_chooser_dialog_new(_("請輸入要匯入的檔案名稱"),
                              GTK_WINDOW(main_window),
                              GTK_FILE_CHOOSER_ACTION_OPEN,
                              GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                              GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
